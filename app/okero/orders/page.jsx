...
export default function AdminOrdersPage() {
  ...
  const [pendingStatusUpdate, setPendingStatusUpdate] = useState({ orderId: null, newStatus: null, stageLabel: null })
  ...
  const updateOrderStatus = async (orderId, newStatus, stageLabel) => {
    // Show confirmation modal instead of directly updating
    setPendingStatusUpdate({ orderId, newStatus, stageLabel })
    setShowConfirmModal(true)
  }

  const confirmStatusUpdate = async () => {
    try {
      setUpdatingOrder(pendingStatusUpdate.orderId)
      ...
      if (result.success) {
        ...
        setOrders(prevOrders => 
          prevOrders.map(order => 
            order.orderId === pendingStatusUpdate.orderId 
        ...
        setPendingStatusUpdate({ orderId: null, newStatus: null, stageLabel: null })
      } else {
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 8px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <Link href="/okero" className="btn" style={{ padding: '8px 12px', fontSize: '14px', textDecoration: 'none' }}>
            Back to Admin
          </Link>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          ...
          {orderStages.map(stage => {
            const count = filteredOrders.filter(order => order.status === stage.value).length
            return (
              <button
                key={stage.value}
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {orderStages.map(stage => {
                      ...
                      return (
                        <button
                          key={stage.value}
                          onClick={() => updateOrderStatus(order.orderId, stage.value, stage.label)}
                          disabled={isDisabled}
                          className="btn"
                          style={{
                            padding: '6px 12px',
      ...
      {showConfirmModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 1rem', fontSize: '1.5rem', color: 'var(--text)' }}>
              Change Order Stage
            </h3>
            <p style={{ margin: '0 0 1.5rem', fontSize: '1rem', color: 'var(--muted)', lineHeight: 1.5 }}>
              Are you sure you want to change the order stage to <strong>{pendingStatusUpdate.stageLabel}</strong>?
              <br />
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
              ...
              <button
                ...
                disabled={updatingOrder === pendingStatusUpdate.orderId}
                style={{
                  padding: '0.75rem 1.5rem',
                  fontSize: '1rem',
                  ...
                  border: 'none',
                  borderRadius: 6,
                  cursor: updatingOrder === pendingStatusUpdate.orderId ? 'not-allowed' : 'pointer',
                  fontWeight: 500,
                  opacity: updatingOrder === pendingStatusUpdate.orderId ? 0.7 : 1
                }}
              >
                {updatingOrder === pendingStatusUpdate.orderId ? 'Updating...' : 'Confirm Change'}
              </button>
            </div>
          </div>
        </div>
